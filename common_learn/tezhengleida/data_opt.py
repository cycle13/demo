...
def tezheng_area(file_dir,newfile_dir,now_time,area_name):
    ...
    CO_sum = df[(df['时间'].isin([now_time]))&(df['区县'].isin([area_name]))]['CO_sum']/co_mean
    SO2_sum= df[(df['时间'].isin([now_time]))&(df['区县'].isin([area_name]))]['SO2_sum']/so2_mean
    NO2_sum = df[(df['时间'].isin([now_time]))&(df['区县'].isin([area_name]))]['NO2_sum']/no2_mean
    PM25_sum = df[(df['时间'].isin([now_time]))&(df['区县'].isin([area_name]))]['PM2.5_sum']/pm25_mean
    PM10_sum = df[(df['时间'].isin([now_time]))&(df['区县'].isin([area_name]))]['PM10_sum']/pm10_mean
    # 写成列表准备写入excel文件
...
def tezheng(file_dir,newfile_dir,now_time):
    ...
    CO_sum = df[(df['时间'].isin([now_time]))]['CO_sum']/co_mean
    SO2_sum= df[(df['时间'].isin([now_time]))]['SO2_sum']/so2_mean
    NO2_sum = df[(df['时间'].isin([now_time]))]['NO2_sum']/no2_mean
    PM25_sum = df[(df['时间'].isin([now_time]))]['PM2.5_sum']/pm25_mean
    PM10_sum = df[(df['时间'].isin([now_time]))]['PM10_sum']/pm10_mean
    # 写成列表准备写入excel文件
    # a = [[so2_mean,no2_mean,co_mean,pm25_mean,pm10_mean],
    #  [so2_std,no2_std,co_std,pm25_std,pm10_std],
    #  [so2_mean_up,no2_mean_up,co_mean_up,pm25_mean_up,pm10_mean_up],
...
def tezheng_hour(file_dir,newfile_dir,now_time):
    df = pd.read_excel(file_dir)
    # 计算占比
    df['CO_sum'] = df['CO']/(df['SO2']+df['NO2']+df['PM2.5']+df['PM10']+df['CO'])
    df['SO2_sum'] = df['SO2']/(df['SO2']+df['NO2']+df['PM2.5']+df['PM10']+df['CO'])
