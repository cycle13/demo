...
def plot_radar(newfile_dir,now_time,name,namell,name2):
    ...
    plt.savefig('image_file/{}/'.format(namell) +name2+ now_time + '.png')
    plt.ylim(0, 20)
    plt.grid(True)
    matplotlib.use('Agg')


def plot_radar_time(newfile_dir,now_time,name,namell,name2):
    data = pd.read_excel(newfile_dir)
    kinds = data.iloc[:, 0]
    labels = data.iloc[:, 1:].columns
    centers = pd.concat([data.iloc[:, 1:], data.iloc[:, 1]], axis=1)
...
def plot_radar_time_hour(newfile_dir,now_time,name,namell,name2):
    data = pd.read_excel(newfile_dir)
    kinds = data.iloc[:, 0]
    labels = data.iloc[:, 1:].columns
    centers = pd.concat([data.iloc[:, 1:], data.iloc[:, 1]], axis=1)
