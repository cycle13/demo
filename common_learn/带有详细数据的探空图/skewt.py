...
class Skew_T(SkewT):
    ...
    def process_skewt(self):
        ...
        pwat = mpcalc.precipitable_water(self.p_i,self.td_i)
        ...
        srh_pos, srh_neg, srh_tot = mpcalc.storm_relative_helicity(self.alt,self.u_i, self.v_i, 1000 * units('m'))
        sbcape, sbcin = mpcalc.surface_based_cape_cin(self.p_i, self.t_i, self.td_i)
        shr6km = mpcalc.bulk_shear(self.p_i, self.u_i, self.v_i, height=self.alt, depth=6000 * units('m'))
        ...
        plt.title('Skew-T Plot \nStation: {} Time: {}'.format(self.st, self.time), fontsize=14, loc='left')
        # Add hodograph
