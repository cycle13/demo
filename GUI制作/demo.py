...
def main_gui():  # 程序主界面
    ...
    win_main = sg.Window('特征雷达图工具箱', layout, font=("宋体", 15))
    while True:
...
def mult_rename_gui(win_main):  # 批量重命名GUI
    ...
    win_main.Hide()  # 主界面隐藏
    layout_mr = [
        [sg.FileBrowse('打开文件', key='filebrowser', target='mr_text1'),
         sg.Combo(['开始时间'], key='choice', size=(15, 1), default_value='12/10/2020'), sg.Button('计算'),
         sg.Button('返回工具箱', key='back_mr')],
        [sg.Text('你选择的文件是:', font=("宋体", 10)), sg.Text('', key='mr_text1', size=(50, 1), font=("宋体", 10))],
        [sg.Text('程序运行记录', justification='center')],
        [sg.Output(size=(70, 20), font=("宋体", 10))]
    ]

    win_mr = sg.Window('时序特征雷达图', layout_mr, font=("宋体", 15), default_element_size=(50, 1))
    while True:
        ...
        if ev_mr is None or ev_mr == 'back_mr':
            ...
            break
        if vals_mr['filebrowser']:
            if vals_mr['choice'] == '12/10/2020':

                # print('您选择的模式为hash值+后缀')
                # print('{0}正在重命名原文件为hash值{0}'.format('*' * 10))
                mr.time_roll_data(vals_mr['filebrowser'])
                print('特征雷达图制作完毕')
            else:
                print('请选择一个重命名模式')
        else:
...
def ciyun_gui(win_main):  # 词云图生成GUI
    ...
    win_main.Hide()
    layout_c = [
        [sg.Text('字体颜色范围：', key='font_color_range'), sg.Text('最小值', key='min'),
         sg.Spin([i for i in range(1, 360)], initial_value=190, key='min_select'), sg.Text('最大值', key='max'),
         sg.Spin([i for i in range(1, 360)], initial_value=250, key='max_select')],
        [sg.InputText('关于字体颜色范围取值（H），可以参考https://www.webfx.com/web-design/color-picker/', key='font_color_range_p',
                      disabled=True)],
        [sg.Text('词云图形状：', key='shape'),
         sg.Combo(['方形', '圆形', '图片形状'], key='shape_select', default_value='图片形状', enable_events=True, size=(8, 1))],
    ]  # 有可能隐藏的控件
    layout_d = [[sg.FileBrowse('打开图片', key='filebrowser', target='image_shape'),
                 sg.InputText('', key='image_shape', disabled=True)]]
    # 有可能隐藏的控件2
    layout = [
        [sg.Text('字体颜色模式：', key='font_color_type'),
         sg.Combo(['图片颜色', '自定义颜色'], key='font_color_type_select', default_value='自定义颜色', size=(12, 1),
                  enable_events=True)],
        [sg.Text('即是按照给定图片的颜色给字上色还是自定义字体颜色', key='font_color_type_p')],
        [sg.Column(layout_c, key='is_visible')],
        [sg.Column(layout_d, key='open_image')],
        [sg.Text('词云图尺寸：', key='size'), sg.Text('宽', key='width'),
         sg.Spin([i for i in range(1, 2001)], initial_value=1000, key='width_select'), sg.Text('高', key='highth'),
         sg.Spin([i for i in range(1, 2001)], initial_value=800, key='higth_select')],
        [sg.Text('字体：', key='font'),
         sg.Combo(['微软雅黑', '楷体', '宋体', '仿宋', '隶书', 'Times New Roman'], key='font_select', default_value='微软雅黑')],
        [sg.Text('背景颜色：', key='back_color'), sg.ColorChooserButton('颜色选择', key='back_color_select'),
         sg.Button('透明色点我', key='color_none')],
        [sg.Text('分词模式：', key='word_split_type'),
         sg.Combo(['自定义模式', '自动分词模式'], key='word_split_type_select', default_value='自动分词模式', size=(8, 1))],
        [sg.Text('注：自定义模式即按照你所给的词组进行绘制，词组之间用空格分隔\n自动分词模式即给定一段话，程序自动进行分词并按照频率进行绘制')],
        [sg.Text('最大词数：', key='max_words'),
         sg.Spin([i for i in range(1, 5001)], initial_value=1000, key='max_words_select'),
         sg.Text('词云图上显示的词数量，退格键删除有问题，建议delete键')],
        [sg.Text('输\n入\n文\n本'), sg.Multiline(size=(30, 10), font=("宋体", 10), key='inputtext'),
         sg.Text('程\n序\n运\n行\n记\n录'), sg.Output(size=(30, 10), font=("宋体", 10))],
        [sg.FolderBrowse('保存路径', key='folderbrowser', enable_events=True, target='backup'),
         sg.InputText('wordcloud.png', key='savefile_name', enable_events=True, size=(15, 1)),
         sg.InputText('', key='file_path', disabled=True, size=(40, 1))],
        [sg.Button('开始生成！', key='generate'), sg.Button('保存图片', key='saveas'), sg.Button('返回工具箱', key='back'),
    ...
    while True:
        ...
        if event == 'generate':
            ...
            try:
                wordcloud = cy.generate(color_type, n, m, font, image_path, bgcolor, w, h, shape, segment_mode, text,
                                        maxword)
            except ValueError:
                print('请输入文本！')
                break
