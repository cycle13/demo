...
def excel_rank(excel_file_dir,excel_filenew_dir,rank_name):
    ...
    df = df.sort_values(by="浓度值1", ascending=True, na_position='last')
    df['排名2'] = df["浓度值2"].rank(method='min', ascending=True)
    df = df.sort_values(by="浓度值2", ascending=True, na_position='last')
    df['排名3'] = df["浓度值3"].rank(method='min', ascending=True)
    df = df.sort_values(by="浓度值3", ascending=True, na_position='last')
    df['排名4'] = df["浓度值4"].rank(method='min', ascending=True)
    df = df.sort_values(by="浓度值4", ascending=True, na_position='last')
    df['排名5'] = df["浓度值5"].rank(method='min', ascending=True)
    df = df.sort_values(by="浓度值5", ascending=True, na_position='last')
    df['排名6'] = df["浓度值6"].rank(method='min', ascending=True)
    df = df.sort_values(by="浓度值6", ascending=True, na_position='last')
