...
class PaintArea(QWidget):#画图类
    ...
    def paintEvent(self, event):#重写绘图事件
        ...
        #计算跑道起始结束坐标
        self.fromX_runway_direction, self.fromY_runway_direction, self.toX_runway_direction, self.toY_runway_direction = self.calculatePoint(self.runway_direction)


        self.painter = QPainter()#画图类
        self.painter.begin(self)
        #画刻度盘
        self.drowLines(self.fromX_runway_direction, self.fromY_runway_direction, self.toX_runway_direction, self.toY_runway_direction,width = self.width()/5)#画跑道
        self.drowLines(20, int(self.height()/2), self.width()-20,int(self.height()/2), width=2, color=QColor(145,146,171))#画水平线
        self.drowLines(int(self.width()/2),20, int(self.width()/2),self.height() - 20, width=2, color=QColor(145,146,171))#画垂直线
