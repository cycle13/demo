...
shp_path=r'haerbin/市界.shp'
proj= ccrs.PlateCarree()  # 简写投影
filename=r'工作簿1.xlsx'
...
olon=np.linspace(125,131,90)
olat=np.linspace(44,47,75)
olon,olat=np.meshgrid(olon,olat)#网格化
func=Rbf(lon,lat,tem,function='Gaussian')#定义径向基函数插值
tem_new=func(olon,olat)#获得插值后的网格气温
print(tem_new)
ax = fig.subplots(1, 1, subplot_kw={'projection': proj})  # 创建子图
extent=[125, 131, 44, 47]#限定绘图范围
...
clip1=maskout2.shp2clip(cs1,ax,r'haerbin/市界.shp','hrb') #白化1
clip2=maskout2.shp2clip(cs2,ax,r'haerbin/市界.shp','hrb') #白化2
...
plt.savefig('plot7.2.png',dpi=60)
plt.show()